...
//    slider->setFixedSize(100,20);
    ...
    ui->slider->setMaximum(9);
    ui->slider->setValue(5);


//
//    label = new QLabel(this);
    ui->label->setText("<font size=7>Status open</font>");

//    label1 = new QLabel(this);
    ui->label1->setText("<font size=3>Offset</font>");
//    label1->setGeometry(QRect(50,80,60,20));

//    label2 = new QLabel(this);
    ui->label2->setText("<font size=3>s</font>");
//    label2->setGeometry(QRect(180,550,20,20));

//    label3 = new QLabel(this);
    ui->label3->setText("<font size=3>senstivity</font>");
    //label3->setGeometry(QRect(180,550,20,20));

//    //time display
//    lcd = new QLCDNumber(this);
    ui->lcd->setSegmentStyle(QLCDNumber::Flat);
    ui->lcd->setFixedSize(180,100);

    ui->lcd->setAutoFillBackground(true);// see the different if you comment that line out.
    QPalette Pal = ui->lcd->palette();
    Pal.setColor(QPalette::Normal, QPalette::WindowText, Qt::red);
    //Pal.setColor(QPalette::Normal, QPalette::Window, Qt::black);
    ui->lcd->setPalette(Pal);



//    lcd1 = new QLCDNumber(this);
    ui->lcd1->setSegmentStyle(QLCDNumber::Flat);
    ui->lcd1->setFixedSize(180,100);
...
//    layout->addWidget(lcd1, 3, 1, Qt::AlignRight);
    ...
    connect(ui->button, SIGNAL(clicked()), this, SLOT(button_callback()));
    connect(ui->button1, SIGNAL(clicked()), this, SLOT(transmitCmd()));
    connect(ui->button2, SIGNAL(clicked()), this, SLOT(button2_callback()));
    ...
    connect(ui->lineedit, SIGNAL(editingFinished()), this, SLOT(my_compensate()));
    connect(ui->lineedit1, SIGNAL(editingFinished()), this, SLOT(my_closetime()));

    connect(ui->slider, SIGNAL(valueChanged(int)), this, SLOT(my_slider()));
...
void Widget::init_plot()
{
    ...
    ui->plot->setMinimumSize(800,200); //we set the minimum size of the plot
    //plot->setFixedSize(500,125);
    ui->plot->setAxisScale(QwtPlot::xBottom,0,600);
    ui->plot->setAxisScaleDraw(QwtPlot::xBottom, new RealScaleDraw(p_count, 1));
    ui->plot->setAxisScaleDraw(QwtPlot::yLeft, new RealScaleDraw(p_count, 2));


    //plot->setTitle("T = f(t)"); //title of the graphe

    // axes names
    ui->plot->setAxisTitle(QwtPlot::xBottom, "<font color = 'blue'>time (s)</font>");
    ui->plot->setAxisTitle(QwtPlot::yLeft, "<font color = 'blue'>Flow velocity (mL/s)</font>");
    ...
    // canvas properties
    ui->plot->canvas()->setLineWidth( 1 );
    ui->plot->canvas()->setFrameStyle( QFrame::Box | QFrame::Plain );
    ui->plot->canvas()->setBorderRadius( 3 );
    ...
    QPalette Pal1 = ui->plot1->palette();
    //Pal.setColor(QPalette::Normal, QPalette::WindowText, Qt::green);
    Pal1.setColor(QPalette::Normal, QPalette::Window, Qt::black);
    ui->plot1->setPalette(Pal1);
    ui->plot1->setMinimumSize(800,200); //we set the minimum size of the plot
    //plot1->setFixedSize(500,125);
    ui->plot1->setAxisScale(QwtPlot::xBottom,0,600);
    ui->plot1->setAxisScaleDraw(QwtPlot::xBottom, new RealScaleDraw(p_count, 1));
    ui->plot1->setAxisScaleDraw(QwtPlot::yLeft, new RealScaleDraw(p_count, 2));

    //plot1->setTitle("T = f(t)"); //title of the graphe

    // axes names
    ui->plot1->setAxisTitle(QwtPlot::xBottom, "<font color = 'blue'>time (s)</font>" );
    ui->plot1->setAxisTitle(QwtPlot::yLeft, "<font color = 'blue'>Volume <br> (mL)</font>");

    //canvas properties
    ui->plot1->canvas()->setLineWidth( 1 );
    ui->plot1->canvas()->setFrameStyle( QFrame::Box | QFrame::Plain );
    ui->plot1->canvas()->setBorderRadius( 3 );

    // creation of the curve (you can add more curve to a graphe)
    curve1 = new QwtPlotCurve;
    curve1->setPen(QPen(Qt::green));
    curve1->attach(ui->plot1);
}
...
{
    ui->button->setDisabled(1);
    ui->button2->setEnabled(1);
    ui->button1->setEnabled(1);
...
void Widget::receive()
{
    ...
    ui->lcd1->display(QString::number( elapsed_time1/1000, 'f', 1));
...
void Widget::addpoint(int point)
{
    ...
    if (data_x->size() == 600){
        ...
        data_ysum->clear();
        p_count++;     //to next 30s
        ui->plot->setAxisScaleDraw(QwtPlot::xBottom, new RealScaleDraw(p_count, 1));  //redraw axes
        ui->plot1->setAxisScaleDraw(QwtPlot::xBottom, new RealScaleDraw(p_count, 1));
    }
    ...
    ui->plot->updateAxes();
    ui->plot->replot(); // we redraw the graphe
    ...
    ui->plot1->updateAxes();
    ui->plot1->replot(); // we redraw the graph
}
...
void Widget::upedge()
{
    ...
    if (value1 == 1) {
        ...
        ui->label->setText("<font size=10>Status \n closed</font>");
...
void Widget::my_timer_func()
{
    ...
    ui->lcd->display(QString::number( elapsed_time/1000.0, 'f', 1));
